<template>
  <div>
    <!-- 卡片视图区域 -->
    <el-card>
      <el-button type="primary" @click="addSongList">新增歌单</el-button>
      <!-- 歌单列表数据 -->
      <el-table :data="songList" border>
        ...
        <el-table-column label="歌单类型" prop="song_list_type"></el-table-column>
    ...
    <el-dialog :title="dialogStatus==1?'添加歌单':'修改歌单'" :visible.sync="isOpen" width="50%">
      <el-form :model="songListForm" label-width="100px" ref="songListFormRef">
        ...
        <el-form-item label="歌单类型" prop="songListType">
          <el-input v-model="songListForm.songListType"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    async loadSongList(){
      let result = await Song.getSongList()
    ...
    handleDelete(id){
      this.$confirm('确认删除此留言？','提示',{
        ...
        let result = await Song.deleteMessage({id})
